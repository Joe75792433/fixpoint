    ...
    while (true) {
        ...
        if (ping == -1) {
            if (in_trouble.count(make_pair(address, prefix_length)) == 0) {
                ...
                count_trouble[make_pair(address, prefix_length)] = 1;
            } else {
                ++count_trouble[make_pair(address, prefix_length)];
            }
        } else {
            if (in_trouble.count(make_pair(address, prefix_length)) == 1) {
                if (count_trouble[make_pair(address, prefix_length)] >= N) {
                ...
                count_trouble.erase(make_pair(address, prefix_length));
            }
        }
    }
